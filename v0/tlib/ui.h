...
static UI_Component ui_component(UI *ui, v2 offset, v2 size, f32 pad) {
    ...
    if (outer.max.x > ui->row.max.x)
        ui->row.max.x = outer.max.x;
    if (outer.max.y > ui->row.max.y)
        ui->row.max.y = outer.max.y;

    if (outer.max.x > ui->window.max.x)
        ui->window.max.x = outer.max.x;
    if (outer.max.y > ui->window.max.y)
        ui->window.max.y = outer.max.y;
