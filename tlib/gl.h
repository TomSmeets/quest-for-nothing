...
static gl_t *gl_init(mem *m, gl_api *api) {
    ...
    gl->empty_image = parse_qoi(
        m,
        (Buffer){
            .ptr = (void *)&GL_ALIEN_IMAGE_DATA[0],
            .size = GL_ALIEN_IMAGE_SIZE,
        }
    );
    ...
    api->glTexImage2D(
        GL_TEXTURE_2D, 0, GL_RGBA16F, gl->empty_image->size_x, gl->empty_image->size_y, 0, GL_RGBA,
        GL_FLOAT, gl->empty_image->data
    );
#endif
...
static void gl_draw(gl_t *gl, Gfx *gfx) {
    ...
    api->glBindVertexArray(gl->vao);
    api->glBufferData(
        GL_ARRAY_BUFFER, gfx->vertex_count * sizeof(gfx->vertex[0]), gfx->vertex, GL_STREAM_DRAW
    );
    api->glBufferData(
        GL_ELEMENT_ARRAY_BUFFER, gfx->index_count * sizeof(gfx->index[0]), gfx->index,
        GL_STREAM_DRAW
    );
    ...
    api->glUniformMatrix4fv(
        gl->shader_uniform_mat, 1, row_major, (GLfloat *)&gfx->world_to_clip.fwd
    );
