...
static f32 snd_cut(f32 v, f32 amount) {
    if (v > 0)
        return f_max(v - amount, 0);
    if (v < 0)
        return f_min(v + amount, 0);
    return 0;
}
...
static f32 snd_play(Sound *snd, f32 dt) {
    ...
    if (0) {
    } else if (snd->time < t_attack)
        volume *= f_remap(snd->time, 0, t_attack, 0, 1);
    else if (snd->time < t_decay)
        volume *= f_remap(snd->time, t_attack, t_decay, 1, snd->adsr_sustain_level);
    else if (snd->time < t_sustain)
        volume *= snd->adsr_sustain_level;
    else if (snd->time < t_release)
        volume *= f_remap(snd->time, t_sustain, t_release, snd->adsr_sustain_level, 0);
