...
static Maze_Cell *maze_get(Maze *maze, v2i pos) {
    ...
    return maze->cells + pos.y * maze->size.x + pos.x;
}
...
static void maze_generate(Maze *maze, v2i pos, Random *rng) {
    // Possible directions
    Maze_Cell open_list[] = {
        Maze_Cell_XN,
        Maze_Cell_XP,
    ...
    for (;;) {
        ...
        // Next position
        v2i next_pos = pos - next_dir;
        Maze_Cell *next = maze_get(maze, next_pos);
        ...
        *next = next_dir;
        ...
        maze_generate(maze, next_pos, rng);
    }
}
