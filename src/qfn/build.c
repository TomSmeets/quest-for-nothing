...
static void build_all(App *app, Cli *cli) {
    bool build = cli_match(cli, "release", "Build qfn release");
    bool upload = cli_match(cli, "publish", "Upload archive");
    if (!build && !upload) return;
